...
class TestChessBoard:
    ...
    def test_knight_neighbors(self):
        board = ChessBoard(size=8)
        result = set(board.knight_neighbors(ChessCoordinate(letter='C', number=3)))
        expected = {
        ...
        result = set(board.knight_neighbors(ChessCoordinate(letter='A', number=1)))
        expected = {
...
class TestSolver:
    def test_solve_statement(self):
        solver = Solver(ChessBoard(size=8))
        result = tuple(
            solver.solve_statement(
                Statement(
